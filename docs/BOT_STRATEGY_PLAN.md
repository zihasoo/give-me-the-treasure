...
S5 가 완성됐고 S4 대비 **56.0% 승률**(100판, 평균 코인 +0.74)을 기록했다. 보류 판단을 이진(성장 코인 증분)에서 연속(예상 획득 턴 당 기대 코인 증분)으로 개선한 결과이며, 무효 환금은 여전히 0건이다. S4 자체는 S3 대비 **79.0%**로 여전히 가장 큰 단일 도약이고, S5는 그 위에 추가 개선을 쌓은 버전이다. 환금 보류(§4.1)·보류 정밀화(§4.1-S5)·상대 견제(§4.2)는 모두 완료되어 §3 구현 완료로 옮겼고, 남은 주요 작업은 **측정 축 보완(§4.3)** 과 **난이도별 노출(§4.4)** 이다. → [§4 우선 작업](#4-우선-작업-로드맵).
...
| `S5BotStrategy` | S4 + 보류 판단 개선(§4.1-S5). **현재 최강 버전.** S4 대비 55.0% 승률. |
| `BotCardEvaluator` | 카드 잠재력 점수(`potentialScore`)·세트 코인(`bestCashCoin`)·버릴 카드(`chooseDiscard`). |
| `CashInPlanOptimizer` | 환금 단계에서 겹치지 않는 세트 조합을 탐색해 총점 최대 계획 생성. `planWithHold`(S4), `planWithHoldS5`(S5) 각각 보류 전략 포함. |
| `BotKind` | 대기실 봇 슬롯 선택지(`S1`·`S2`·`S3`·`S4`·`S5`·`HEURISTIC` 노출). |
...
> **S5 완성. 남은 순서: 4.3 → 4.4 → 4.5.** 4.1·4.1-S5·4.2 는 완료되어 §3(구현 완료 → 완료 상세)으로 옮겼다(S4 vs S3 79%, S5 vs S4 56%, 무효 환금 0 확인). 이제 측정 보완 및 난이도 노출이 다음 과제다.

### 4.3 [현재 우선] 측정 축 교체 — 정책 대결 + 사람 평가

봇 vs 봇 승률은 **회귀 안전망**(무효 환금 0 검증)으로만 남기고, "강함"의 척도를 바꾼다:

- **정성 평가:** 직접 몇 판 붙어 "쪼잔한 즉시 환금/무방비 분할"이 사라졌는지 눈으로 확인. 체감은 결국 사람이 본다.
- **보조 지표 로깅:** 평균 환금 세트 크기, 게임당 환금 횟수, 상대 대형 세트 허용 횟수 — 승률 한 숫자보다 정책 변화를 잘 보여준다.
