...
### 2. 내가 설명한다 그냥
1. 환금 상황에서 당장 한도가 넘치는게 아니면 환금을 최대한 미루는게 무조건 이득임 (다음 분배에서 카드가 넘쳐도 그 턴의 환금까지는 카드를 모두 보유하고 있을 수 있기 때문. 또한 꾀부리기나 분배 단계에서 견제해야 하는 카드가 많아져 상대에게 상당한 부담이 됨)
2. 꾀부리기를 할 때 견제 조건은 많지만, 가장 중요한건 상대가 같은 색의 연속된 숫자를 모으지 못하게 세트를 쪼개서 분배하는 것임 (또는 뒷면으로 숨겨서 이용)
3. 
