...
더미)로 재구성한다(이벤트 구독 없음). `S7`은 회귀 baseline으로 클래스만 남고 대기실에는 `S8`만 노출한다.

S7 회귀 체온계(자가대전 400판): **S8 53.0% vs S7**, 평균 코인 28.54 vs 27.82, 무효 환금 0건(2026-06-17).
강함의 주 지표는 여전히 사람 vs 봇 로그다. 4.3 상대 모델의 **관찰/학습**과 4.7 난이도별 봇·4.8 lookahead는 후속.
...
| `BotKind` | 대기실 봇 슬롯 선택지. 현재 `S8`만 노출, 향후 난이도 확장 예정. |
...
**현재 한계:** S8은 분할/선택/환금과 2v2 분배까지 다루지만, 상대 모델은 아직 prior 고정(관찰/학습 미배선)이고, 환금 단계 자체는 검증된 S7 경로(`Tuning.S8`=S7 수치)를 공유한다. lookahead(4.8)와 난이도별 봇(4.7)은 미구현이다.
...
> **남은 일(미착수).** 현재 `BotKind`는 `S8` 하나만 노출. 강한 봇(S8)이 섰으니 이제 가능.
